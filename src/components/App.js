...
class App extends React.Component {
    ...
    onSearchSubmit = async (event) => {
            try {
                const result = await fetch (`https://www.metaweather.com/api/location/search/?query=${event}`);
                const location_rep = await result.json();
                const cur_woeid = location_rep[0].woeid;
                const woeid_rep = await fetch (`https://www.metaweather.com/api/location/${cur_woeid}/`);
